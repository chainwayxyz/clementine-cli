...
fn get_storage_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    get_storage_dir_with_base(None)
}

/// Get the storage directory path with optional custom base directory
fn get_storage_dir_with_base(
    ...
    Ok(base.join(".clementine").join("keys"))
}
