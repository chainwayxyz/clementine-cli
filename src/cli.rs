...
pub async fn cli_scan_withdrawals(
    ...
    if available_utxos.is_empty() {
        ...
        let print_withdrawal_cmd = |outpoint: &OutPoint| {
            println!(
                "$ clementine-cli withdraw sign --network {} {} {} {}",
                config.network,
                &signer_address.address_with_prefix(),
                destination_address,
                outpoint,
...
pub fn cli_list_all_deposit_addresses() -> Result<(), BridgeCliError> {
    ...
    if deposits.is_empty() {
        println!("{} No stored deposit addresses found.", "INFO".bold());
        println!("\nTo generate a deposit address, use:");
        println!("  clementine-cli deposit generate-address <RECOVERY_ADDRESS> <CITREA_ADDRESS>");
    } else {
        println!("{} Stored Deposit Addresses:", "INFO".bold());
        println!();
        for (idx, d) in deposits.iter().enumerate() {
            println!("{}. Address: {} | Network: {:?}", 
                idx + 1, 
                d.deposit_address.to_string().bold(), 
                d.network
            );
        }
        println!();
        println!("Use 'clementine-cli deposit show-address-details <ADDRESS>' to view full details.");
    }
    ...
    match details {
        None => {
            println!(
                ...
                "WARNING".bold(),
                deposit_address.bold()
            );
            println!("\nThis address may not have been generated by this CLI instance.");
        }
        Some(d) => {
            println!("{} Deposit Address Details", "INFO".bold());
            println!();
            println!("  Deposit Address:           {}", d.deposit_address.to_string().bold());
