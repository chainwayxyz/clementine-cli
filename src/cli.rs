...
use crate::{
    ...
    wallet::{
        ...
        wallet_utils::{
            ...
            derive_and_validate_mnemonic_import, validate_wallet_availability,
        },
    },
...
pub async fn cli_import_wallet_from_mnemonic(
    ...
    let _derived_address = derive_and_validate_mnemonic_import(network, label, purpose, &mnemonic, None)
        .await?;
...
pub async fn deposit_create_signed_recovery_tx(
    ...
    let tx = deposit::create_signed_recovery_tx(recovery_params, config, keypair, None).await?;
