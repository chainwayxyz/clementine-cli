...
pub fn parse_address(address: &str, network: Network) -> Result<BitcoinAddress> {
    use crate::{BitcoinAddress, NetworkUnchecked};

    let unchecked_address: BitcoinAddress<NetworkUnchecked> = address
        .parse()
        .map_err(|_| eyre!("Invalid Bitcoin address format"))?;
...
pub fn parse_taproot_address(address: &str, network: Network) -> Result<BitcoinAddress> {
    ...
    if address.address_type() != Some(AddressType::P2tr) {
        return Err(eyre!("Address is not a taproot (P2TR) address"));
    }
...
pub fn get_all_wallet_addresses() -> Result<()> {
    use std::fs;

    let storage_dir = crate::wallet_storage::get_storage_dir()
        .map_err(|e| eyre!("Failed to get storage directory: {}", e))?;
